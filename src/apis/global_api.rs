...
pub async fn get_status(configuration: &configuration::Configuration, ) -> Result<models::GetStatus200Response, Error<GetStatusError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::GetStatus200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::GetStatus200Response`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn register(configuration: &configuration::Configuration, register_request: Option<models::RegisterRequest>) -> Result<models::Register201Response, Error<RegisterError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::Register201Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::Register201Response`")))),
        }
    } else {
        let content = resp.text().await?;
